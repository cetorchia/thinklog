 ...
 *
 *  Modification (c) 2012 Carlos E. Torchia
 *      - node.drag == true if we allow dragging of this node
 *      - node.url == true if url is opened when node is clicked
 *      - node.size == font-size, if any
 /*--------------------------------------------------------------------------*/
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function(node) {
		...
		var text = r.text(0, 0, node.label || node.id);
                ...
                if(node.url) {
                    shape.mouseup(function() { window.open(node.url, "_self"); });
                }
                return shape;
            }
        }
        ...
        shape.items.forEach(function(item){
            ...
            if(node.url) {
                ...
                item.node.style.cursor = "default";
            }
        });
        if(node.drag) {
